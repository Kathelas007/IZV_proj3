...
def plot_geo(gdf: geopandas.GeoDataFrame, fig_location: str = None, show_figure: bool = False):
    ...
    for i in range(2):
        gdf[gdf["p5a"] == i + 1].plot(ax=axes[i], markersize=3, color=colors[i], alpha=0.4)
        ctx.add_basemap(axes[i], crs=gdf.crs.to_string(), source=ctx.providers.Stamen.TonerLite, zoom=10, alpha=0.9)
...
def plot_cluster(gdf: geopandas.GeoDataFrame, fig_location: str = None, show_figure: bool = False):
    ...
    ctx.add_basemap(ax, crs=gdf.crs.to_string(), source=ctx.providers.Stamen.TonerLite, zoom=10, alpha=0.9)
